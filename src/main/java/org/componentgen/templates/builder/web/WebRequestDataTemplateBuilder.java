...
public class WebRequestDataTemplateBuilder extends AbstractTemplateBuilder {
    public WebRequestDataTemplateBuilder(ComponentTemplate componentTemplate, ComponentNamingContext namingContext){
    ...
    protected ClassMetadata buildClassMetadata(PackageMetadata parentPackage){
        if(namingContext.getApiType() == APITYPE.COMMAND) {
            ...
            ClassMetadata webRequest = new ClassMetadata(namingContext.getWebRequestDataClassName(), AccessModifier.PUBLIC, null, packagesToImportForWebRequest, "JsonApiResource", null, false, CLASSTYPE.CLASS);
            webRequest.setExtendsNarrow(namingContext.getWebRequestAttributesClassName());
